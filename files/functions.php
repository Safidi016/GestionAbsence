...
function inscriptionEtudiant($id_user, $cin, $nom, $cne, $prenom, $date_naissance, $adresse, $lieu_naissance, $telephone, $email,$niveau)
{

    $db = database();

    $sttm = $db->prepare("INSERT INTO etudiant (id_user, cin, nom, cne, prenom, date_naissance, adresse, lieu_naissance, telephone, email, Niveau) VALUE (:id_user, :cin, :nom, :cne, :prenom, :date_naissance, :adresse, :lieu_naissance, :telephone, :email)");
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["type"]) && !empty($_POST["nom"]) && !empty($_POST["prenom"]) && !empty($_POST["email"]) && !empty($_POST["telephone"])) {
            ...
            if ($id_user != null) {
                if ($_POST["type"] == "etudiant") {
                    ...
                    if (inscriptionEtudiant($id_user, $_POST["cin"], $_POST["nom"], $_POST["cne"], $_POST["prenom"], $_POST["date_naissance"], $_POST["adresse"], $_POST["lieu_naissance"], $_POST["telephone"], $_POST["email"],$_POST["Niveau"])) {
...
if (isset($_POST["modifier_etudiant"])) {
    ...
    if (!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['telephone']) && !empty($_POST['cin']) && !empty($_POST['cne']) && !empty($_POST['date_naissance']) && !empty($_POST['lieu_naissance']) && !empty($_POST['adresse']) && !empty($_POST['email']) && !empty($_POST['username']) && !empty($_POST['password'])) {
        ...
        if(update_student($etd))
        {
            ...
            header("Location: etudiants.php?id=".$etd['id']);
            exit();
...
/////////////////////par etu diant
if (isset($_POST["modifier_etudiantt"])) {
    ...
    if (!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['telephone']) && !empty($_POST['cin']) && !empty($_POST['cne']) && !empty($_POST['date_naissance']) && !empty($_POST['lieu_naissance']) && !empty($_POST['adresse']) && !empty($_POST['email']) && !empty($_POST['username']) && !empty($_POST['password'])) {
        ...
        {
            $_SESSION["message"] = "Les infos de l'étudiant ont été bien modifier";
            header("Location: index.php?id=".$etd['id']);
            exit();
...
function deleteEtudiant($id)
{
    ...
    $sttm = $db->prepare("DELETE FROM etudiant WHERE id=:id ");
    $sttm->bindParam(":id", $id);
...
function Toutclasse()
{
    ...
    $sttm = $db->prepare("SELECT * FROM classe ");
    if ($sttm->execute()) {
        $classe = $sttm->fetchAll();
        return $classe;
    }
...
function getClasseByID($id)
{
    ...
    $classe = "";
    $sttm = $db->prepare("SELECT * FROM classe WHERE id_classe = :id;");
    $sttm->bindParam(':id', $id);
    if ($sttm->execute()) {
        $classe = $sttm->fetch(PDO::FETCH_ASSOC);
        return $classe;
    }
    return $classe;
}
...
if (isset($_POST["modifier_classe"])) {
    ...
    if (!empty($_POST['code']) && !empty($_POST['nom_classe'])) {

        $db = database();

        $sttm = $db->prepare("UPDATE classe SET code = :code, nom_classe = :nom_classe WHERE id_classe = :id_classe");
        $sttm->bindParam(':id_classe', $_POST['id_classe']);
        $sttm->bindParam(':code', $_POST['code']);
        $sttm->bindParam(':nom_classe', $_POST['nom_classe']);

        if($sttm->execute())
        {
            $_SESSION["message"] = "Les infos de la classe ont été bien modifier";
            header("Location: classe.php?id=". $_POST['id']);
            exit();
...
function deleteClasse($id)
{
    ...
    $sttm = $db->prepare("DELETE FROM classe WHERE id_classe=:id_classe ");
    $sttm->bindParam(":id_classe", $id);
...
//////modifier prof
function getProfToModify($id)
{
    $db = database();

    $professeur = "";
    $sttm = $db->prepare("SELECT professeur.*, utilisateur.login, utilisateur.password
            FROM professeur
            INNER JOIN utilisateur
            ON professeur.id_user=utilisateur.id
            WHERE professeur.id_user = :id;");
    $sttm->bindParam(':id', $id);
    if ($sttm->execute()) {
        $professeur = $sttm->fetch(PDO::FETCH_ASSOC);
        return $professeur;
    }
    return $professeur;
}

if (isset($_GET["modifier_prof"])) {
    if (!empty($_GET['id']) && !empty($_GET['nom']) && !empty($_GET['prenom']) && !empty($_GET['som']) && !empty($_GET['telephone']) && !empty($_GET['email']) && !empty($_GET['username']) && !empty($_GET['password'])) {

        $etd['id'] = $_GET['id'];
        $etd['nom'] = $_GET['nom'];
        $etd['prenom'] = $_GET['prenom'];
        $etd['som'] = $_GET['som'];
        $etd['email'] = $_GET['email'];
        $etd['telephone'] = $_GET['telephone'];
        $etd['username'] = $_GET['username']; // Utilisation de 'username' au lieu de 'login'
        $etd['password'] = $_GET['password'];

        if (update_prof($etd)) {
            $_SESSION["message"] = "Les infos ont été bien modifiées"; // Correction de la phrase en français
            header("Location: index.php?id=" . $etd['id']);
            exit();
        } else {
            $_SESSION["message"] = "Erreur lors de votre action";
...
function update_prof($etd)
{
    $db = database();

    $sttm = $db->prepare("UPDATE professeur
        JOIN utilisateur ON professeur.id_user = utilisateur.id
        SET utilisateur.login = :login, utilisateur.password = :password, professeur.nom = :nom, professeur.prenom = :prenom, professeur.som = :som, professeur.email = :email, professeur.telephone = :telephone WHERE professeur.id = :id");

    $sttm->bindParam(':id', $etd['id']);
    $sttm->bindParam(':som', $etd['som']);
    $sttm->bindParam(':nom', $etd['nom']);
    $sttm->bindParam(':prenom', $etd['prenom']);
    $sttm->bindParam(':telephone', $etd['telephone']);
    $sttm->bindParam(':email', $etd['email']);
    $sttm->bindParam(':login', $etd['username']);
    $sttm->bindParam(':password', $etd['password']);
